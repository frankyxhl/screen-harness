...
from .render import DrawBox, render_video
...
def start_recording(name: str, *, screen: str | None = None, mic: str | None = None) -> Path:
    ...
    timeline = Timeline.create(path=recording_dir / "timeline.json", recording_id=recording_id, title=name, source_video="raw.mp4")
...
def step(title: str, *, t: float | None = None) -> None:
    _timeline().add_event("step", t=_event_time(t), title=title)
...
def generate_sop_captions(recording_dir: Path | None = None):
    return generate_caption_assets(recording_dir or _recording_dir())
...
def render(recording_dir: Path | None = None) -> Path:
    ...
    outputs = generate_caption_assets(directory)
