...
def test_bdd_training_template_creates_professional_video_assets(tmp_path):
    timeline = Timeline.create(
        ...
        recording_id="chrome_demo",
        title="Chrome GitHub Demo",
        source_video="raw.mp4",
        intro={
            "title": "This video demonstrates the Screen Harness GitHub repository",
            "subtitle": "Open Chrome and inspect the project page.",
            "countdown": 5,
        },
    )
    timeline.add_event("chapter", t=0.0, title="Repository walkthrough")
    timeline.add_event("step", t=1.0, title="Open Chrome", note="Launch the browser in a clean window.")
    timeline.add_event("step", t=4.0, title="Visit repository", note="Open the Screen Harness repository on GitHub.")
    timeline.add_event("caption", t=1.2, text="Open Chrome and navigate to the repository.", duration=3.0)
    ...
    assert "01  Open Chrome" in main_ass
    ...
    assert "# Chrome GitHub Demo" in markdown
